...
4. **建立证据表**：按审核维度记录字段、窗口、可见事实和缺失信息。
5. **执行审核**：
...
6. **形成发现**：每条发现必须能指回具体字段或数据；不能验证的判断标为人工确认或信息缺口。
7. **聚合结论**：按 `references/audit-policy.md` 的结论规则生成通过、不通过、信息不足或需人工确认。
8. **编写输出**：读取 `references/output-contract.md`；批量审核时逐条生成独立报告，修改建议应简短、具体、适合学习者执行。
9. **处理后续**：
   - 平台操作读取 `references/platform-operations.md`；结论为“通过”且审核员身份明确时，直接执行分配审核并核验任务状态。
