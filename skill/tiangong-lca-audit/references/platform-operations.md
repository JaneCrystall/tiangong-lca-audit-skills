...
审核结论确认为“通过”后，直接进入分配审核，不再等待单独的分配指令：

1. 确认报告结论为“通过”，且平台退回意见为“无”。
2. 操作前读取任务，确认 `state_code=0` 且 `reviewer_id` 为空。
3. 将任务分配给任务指定的审核员账号；未指定时使用环境中配置的审核员账号。审核员身份无法唯一确认时停止写入。
4. 调用 `cmd_review_assign_reviewers`，一次处理一条审核任务。
5. 操作后重新读取任务，确认 `state_code=1` 且 `reviewer_id` 包含目标审核员。
6. 审核员账号创建或上传审核报告流，在待审核数据中填写验证审查和合规性声明并关联报告流，然后保存并提交。
7. 管理员在已分配任务中复核并确认通过。

分配审核只是把通过数据送入审核员通道，不等于管理员最终通过。
...
4. 使用当前审核员账号上传 DOCX 到 `external_docs/{uuid}.docx`；不得用非分配审核员账号创建报告文件。
...
- 审核任务中形成“通过”结论且审核员身份已明确时，按上述通过流程直接执行分配审核。
